...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
      ...
      <xsl:when test="go">
        ...
        <xsl:choose>
          <xsl:when test="go[substring(@href,1,2)!='#']">
            <form action="{go/@href}" method="{go/@method}"
  ...
  <xsl:template match="card">
    <div>
      ...
      <xsl:attribute name="class"><xsl:if test="count(/wml/card) = 1">onlycard</xsl:if> card</xsl:attribute>
      <xsl:if test="@title!=''">
              ...
              <xsl:choose>
                  <xsl:when test="go">
                      <script type="text/javascript">
                          nsWMLRegisterTimer('go' ,'<xsl:value-of select="go/@href"/>');
                      </script>
                  </xsl:when>
                  <xsl:otherwise>
                      <p><i>Warning: this WML page uses a construct 'onevent' with
                              an action '<xsl:value-of select="local-name(./*[1])"/>' not recognised by wmlbrowser</i></p>
                  </xsl:otherwise>
              </xsl:choose>
          </xsl:when>
          <xsl:otherwise>
              <p><i>Warning: this WML page uses a construct 'onevent' with type
              '<xsl:value-of select="@type"/>' not supported by wmlbrowser</i></p>
          </xsl:otherwise>
      </xsl:choose>
  ...
  <xsl:template match="anchor[./go]">
    <xsl:apply-templates select="go/setvar" mode="setvar" />
    <form action="{go/@href}" onsubmit="return nsWMLcheckForm(this);">
  ...
  <xsl:template match="input">
      <xsl:element name="input">
          ...
          <xsl:for-each select="@type | @value | @size | @maxlength | @tabindex | @title | @accesskey">
              <xsl:copy />
          </xsl:for-each>
      </xsl:element>
  </xsl:template>
  ...
  <xsl:template match="select">
    <select id="{@name}" onchange="nsWMLcheckSelect(this);">
