...
var
  ...
  Tmp: boolean;
begin
  with (FControl as TListBox) do begin
    Clear;
    for I:= 0 to TheOptions.Count - 1 do begin
...
begin
  if not Assigned(FControl.Parent) then begin
  ...
  with (FControl as TListBox) do
    if (Index < Items.Count) then begin
      ...
      if MultiSelect and Selected[Index] or
        not MultiSelect and (ItemIndex = Index) then begin
        S:= Self.Name + '=';
  ...
  for I:= 0 to L.Count - 1 do
    with TAttribute(L[I]) do
      case Which of
        ...
        ColsSy: Cols:= Value;
        WrapSy:
          if (Lowercase(Name) = 'soft') or (Lowercase(Name) = 'hard') then begin
            SB:= ssVertical;
            Wrap:= True;
          end;
      end;
...
begin
  with (FControl as TMemo) do begin
    Lines:= TheText;
    SelStart:= 0;
    SelLength:= 0;
  end;
...
begin
  for I:= 0 to Count - 1 do
    if TFormControlObj(Items[I]).Pos = Posn then begin
      Result:= Items[I];
      Exit;
    end;
  Result:= nil;
