...
// delete all items from image list, but leave the list
  ...
  for i:= 0 to (fImageDescriptorList.Count - 1) do begin
...
// release color tables, but keep the list
  ...
  for i:= 0 to (fColorTableList.Count - 1) do begin
...
// read extensions and image data until end of file
  ...
  while (not done) do try {LDB}
    ...
    if (b = 0) then {// just skip this?} begin
      b:= 0;
      Done:= True; {LDB}
    end
    else if (b = kGifTerminator) then {// got it all} begin
      done:= true;
    end
    else if (b = kGifImageSeparator) then {// next bitmap} begin
      ReadImageDescriptor;
    end
    else if (b = kGifExtensionSeparator) then {// special operations} begin
      ReadExtension(Done);
    end
    else {// unknown} begin
      GIF_Error(4);
...
var
  i, n: integer;
begin
  with fScreenDescriptor^ do begin
...
begin
  ...
  if (Size > 0) then {// OK, a table does exist} begin
    ...
    for i:= 0 to (ct^.rSize - 1) do {// read a triplet for each TColor} begin
...
// read data blocks until the end of the list
  ...
  while (b > 0) do {// more blocks to get?} begin
...
// a graphic extension
    ...
    if (rLabel = kGifLabelGraphic) then begin
...
// a comment extension
    ...
    else if (rLabel = kGifLabelComment) then begin
...
// a plain text extension
    ...
    else if (rLabel = kGifLabelText) then begin
...
// an application extension
    ...
    else if (rLabel = kGifLabelApplication) then begin
      ReadSourceInteger(1, rApp.rBlockSize); // block size
      if (rApp.rBlockSize <> 11) then begin
...
// parse next code from BitString
    ...
    oc:= pc;
    Done:= False;
    while (pc <> rEndCode) and not Done do begin
...
begin
  with fZipData^ do begin
    for i:= 0 to (kGifCodeTableSize - 1) do begin
      rPrefix[i]:= 0;
      rSuffix[i]:= 0;
    end;
...
// bumping Y ?
    ...
    if (rCurX >= rID^.rWidth) then begin
      rCurX:= 0;

// if not interlaced image, then just move down the page

      if (not rID^.rInterlaced) then begin
        rCurY:= rCurY + 1;
      end
...
// if past the end of the bitmap, start next pass
        ...
        if (rCurY >= rID^.rHeight) then begin
          rCurPass:= rCurPass + 1;
...
// get next code from the stack, and index into PixelList
      ...
      if ((0 <= j) and (j < rID^.rPixelCount)) then begin
    ...
    if ((id <> nil) and (id^.rBitmap = nil)) then // don't do it again
      with id^ do begin
        ...
        try
          ...
          for Y:= rHeight - 1 downto 0 do begin
            P:= Pix + (Y * FullWidth);
            for X:= 0 to rWidth - 1 do begin
...
// is bitmap transparent?
        ...
        if ((0 <= TrIndex) and (TrIndex < ct^.rSize)) then begin
...
{find size needed for strip bitmap}
  ...
  try
    ...
    with PL^.BFH do begin {set up the bitmap file header}
      bfType:= 19778;
      bfSize:= FileSize;
      bfReserved1:= 0;
      bfReserved2:= 0;
    ...
    with PL^.BIH do begin {and the bitmap info header}
      biSize:= Sizeof(TBitmapInfoHeader);
    ...
    if IsTransparent then begin {set up a mask similarly}
    ...
    for i:= 0 to (fImageDescriptorList.Count - 1) do {for all the frames} begin
      id:= fImageDescriptorList.Items[i];
      if (id <> nil) then
        with id^ do begin
          ct:= fColorTableList.Items[rLocalColorTable];
          ...
          for Y:= Height - 1 downto IntMax(Height - rHeight, ImageTop[I]) do begin
          {find the start of each frame row in destination.  Note that the source
           ...
           imagetop and imageleft}
            ...
            for X:= 0 to rWidth - 1 do begin
              if P < PRight then {prevent write beyond proper right side in case rwidth to wide} begin
            ...
            {for frames after the 0th, only the non transparent pixels are written
             as writing transparent ones might cover results copied from the previous frame}
                ...
                else if i = 0 then begin {transparent pixel, first frame, write black}
                  P^:= #0;
                  Inc(P);
                  P^:= #0;
                  Inc(P);
                ...
                if IsTransparent then {also do the mask} begin
    ...
    {Now copy this frame to the next (unless it the last one).  This serves as a
     ...
     disposal method but will be fixed up for dtBackground below}
      if (i < fImageDescriptorList.Count - 1) then begin
        for Y:= Height - 1 downto 0 do begin {copy line by line}
      ...
      {for dtBackground, fill the mask area occupied by the current copied image with
       ...
       (although the next image will no doubt write on part of this area.}
        if IsTransparent and (ImageDisposal[i] in [2, 3]) then
          {dtToPrevious run as dtBackground as it seems other browsers do this}
          with id^ do
            for Y:= Height - 1 downto IntMax(Height - rHeight, ImageTop[I]) do begin
...
begin
  gx:= nil;
  id:= fImageDescriptorList.Items[image];
  if (id^.rExtensionList <> nil) then begin
    for n:= 0 to (id^.rExtensionList.Count - 1) do begin
      ex:= id^.rExtensionList.Items[n];
      if ((ex^.rLabel = kGifLabelGraphic) and (gx = nil)) then begin
        gx:= @(ex^.rGraphic);
      end;
    end;
  end;
...
begin
  n:= -1;
  for i:= 0 to (ct^.rSize - 1) do begin
