...
begin
  if Assigned(Img) then begin
    ...
    if (Img.ObjAlign = ALeft) then begin
      IR:= IndentRec.Create;
      with IR do begin
    ...
    else if (Img.ObjAlign = ARight) then begin
      IR:= IndentRec.Create;
      with IR do begin
...
begin
  if Source is TMyFont then begin
    ...
    with TAttribute(L[I]) do
      case Which of
        ...
        WidthSy: if System.Pos('%', Name) = 0 then
            SpecWidth:= Value
          else if (Value > 0) and (Value <= 100) then begin
            SpecWidth:= Value;
            PercentWidth:= True;
          end;
...
begin
  ...
  else if (Image is TBitmap) then begin
...
begin
  ...
  if (Image = DefBitmap) then begin
    ...
    if not ImageKnown then begin {need to get the dimensions}
      Reformat:= True;
    end;
  end;
end;
...
begin
  ...
  TmpImage:= Image;
  if ViewImages and not Assigned(TmpImage) then begin
    if Source <> '' then
      with SectionList do begin
        if not Assigned(GetImage) then
          Source:= (TheOwner as ThtmlLite).HTMLExpandFilename(Source)
        else if Assigned(ExpandName) then begin
    ...
    if not Assigned(TmpImage) then begin
      if Delay then begin
        Image:= DefBitmap;
        TmpImage:= DefBitmap;
    ...
    else if TmpImage is TGifImage then begin
      if FromCache then begin {it's in Cache already, make copy}
  ...
  if TmpImage is TGifImage then begin
    ImHeight:= TGifImage(TmpImage).Height;
    ImWidth:= TGifImage(TmpImage).Width;
  end
  else if TObject(TmpImage) is TGPImage then begin
  ...
  if not ImageKnown then
    if not ((Image = ErrorBitmap) or (TmpImage = DefBitmap)) then begin
      if PercentWidth then begin
        ObjWidth:= MulDiv(AvailableWidth, SpecWidth, 100);
        if SpecHeight <> 0 then ObjHeight:= SpecHeight
        else ObjHeight:= ImHeight;
      end
      else if (SpecWidth <> 0) and (SpecHeight <> 0) then begin {Both width and height specified}
        ObjHeight:= SpecHeight;
        ObjWidth:= SpecWidth;
        ImageKnown:= True;
      end
      else if SpecHeight <> 0 then begin
        ObjHeight:= SpecHeight;
        ObjWidth:= MulDiv(SpecHeight, ImWidth, ImHeight);
        ImageKnown:= True;
      end
      else if SpecWidth <> 0 then begin
        ObjWidth:= SpecWidth;
      ...
      if (SpecHeight <> 0) and (SpecWidth <> 0) then begin {Both width and height specified}
        ObjHeight:= SpecHeight;
        ObjWidth:= SpecWidth;
...
begin
  with ParentSectionList do begin
    ViewImages:= ShowImages;
    Dec(TopY, YOff);
    Dec(YBaseLine, YOff);
  end;
  if ViewImages then begin
    TmpImage:= Image;
  ...
  with Canvas do begin
    ...
    if SubstImage and not HasBlueBox then begin
...
begin
  for I:= 0 to Count - 1 do
    if TFloatingObj(Items[I]).Pos = Posn then begin
      Result:= Items[I];
      Exit;
    end;
  Result:= nil;
  ...
  for I:= 0 to Count - 1 do begin
    ...
    if Obj is TImageObj then
      with TImageObj(Obj) do begin
        ...
        if (IX >= 0) and (IX < LimX) and (LIY >= 0) and (LIY < LimY) then begin
          IY:= LIY;
          Result:= True;
          AMap:= IsMap;
          Posn:= Pos;
          ...
          if UseMap then
            with ParentSectionList.MapList do
              for J:= 0 to Count - 1 do begin
                MapItem:= Items[J];
                if MapItem.MapName = MapName then begin
                  UMap:= True;
                  Exit;
                end;
              end;
...
var
  ...
  S: string;
begin
  if Assigned(MasterList.SubmitForm) then begin
    SL:= TStringList.Create;
    for I:= 0 to ControlList.Count - 1 do
      with TFormControlObj(ControlList.Items[I]) do begin
        J:= 0;
        while GetSubmission(J, S) do begin
          if S <> '' then
            SL.Add(S);
          Inc(J);
        end;
...
begin
  ...
  Pos:= Position;
  MasterList:= AMasterList;
  with (MasterList.Parser as ThlParser) do begin
...
begin
  if Assigned(FControl) then {hidden controls are Nil} begin
...
begin
  ...
  with MasterList.FormControlList do begin
    for I:= 0 to Count - 1 do
      with TFormControlObj(Items[I]) do
        if not ShowIt and Assigned(FControl) then begin
...
begin
  Result:= False;
  if (Index <= 1) and (XPos >= 0) then begin
    S:= '';
    if Name <> '' then S:= Name + '.';
...
begin
  inherited Create(AMasterList, Position, L);
  if Typ = 'submit' then begin
    Which:= Submit;
    if Value = '' then
      Value:= 'Submit';
  end
  else if Typ = 'reset' then begin
    Which:= ResetB;
    if Value = '' then
      Value:= 'Reset';
  end
...
begin
  if (Index = 0) and TCheckBox(FControl).Checked then begin
    Result:= True;
    S:= Name + '=' + Value;
  end
  else Result:= False;
...
begin
  if (Index = 0) and RButton.Checked then begin
    Result:= True;
    S:= Name + '=' + Value;
  end
  else Result:= False;
...
begin
  ...
  H:= 0;
  for I:= 0 to Count - 1 do
    with TSectionBase(Items[I]) do begin
      if (Y >= H) and (Y < H + DrawHeight) then begin
...
begin
  if Assigned(ActiveLink) or Assigned(ActiveImage) then
    PPanel.Invalidate;
  if Assigned(ActiveLink) then begin
    ActiveLink.Hover:= False;
    ActiveLink:= nil;
  end;
  if Assigned(ActiveImage) then begin
...
begin
  ...
  for I:= 0 to AGifList.Count - 1 do
    with TGifImage(AGifList.Items[I]) do begin
...
begin
  ...
  with FormControlList do
    if not ActiveInList then begin {if none of the formcontrols are active, turn off tabs for those off screen}
      for I:= 0 to Count - 1 do
        with TFormControlObj(Items[I]) do
...
begin
  ...
  if (I = -1) and (J >= 0) then begin
    ...
    if Assigned(Image) then {put in Cache} try
  ...
  if (I >= 0) or Assigned(Image) then {a valid image in the Cache} begin
    while J >= 0 do begin
      Obj:= MissingImages.Objects[J];
      if (Obj = Self) then
        BitmapLoaded:= False {the background image, set to load}
      else if (Obj is TImageObj) then begin
...
begin
  ...
  if BMName <> '' then begin
    ...
    I:= BitmapList.IndexOf(UName); {first see if the bitmap is already loaded}
    if I > -1 then begin {yes, handle the case where the image is already loaded}
      Result:= BitmapList.GetImage(I);
      FromCache:= True;
      if Result is TBitmap then
        with BitmapList.Objects[I] as TBitmapItem do begin
          if Transp = TGif then
            Transparent:= TGif {it's a transparent GIF}
          else if Transparent = LLCorner then begin
  ...
  {The image is not loaded yet, need to get it}
    Result:= nil;
    if Assigned(GetImage) then begin {the OnImageRequest}
      Stream:= nil;
      GetImage(TheOwner, BMName, Stream);
      if Stream = WaitStream then
        Delay:= True
      else if Assigned(Stream) and (Stream.Memory <> nil) and (Stream.Size >= 1) then begin
        NonAnimated:= True;
        if KindOfImage(Stream.Memory) in [GIF, Gif89] then
    ...
    else begin {look for the image file}
    ...
    if Assigned(Result) then {put in Image List for use later also} try
      if Assigned(AMask) then Tr:= Transparent
      else Tr:= NotTransp;
...
var
  ...
  Rslt: string;
begin
  if ShowImages and not BitmapLoaded and (BitmapName <> '') then begin
    if not Assigned(BackgroundBitmap) then begin
      Dummy1:= NotTransp;
      if not Assigned(GetImage) then
      ...
      if (TmpResult is TBitmap) or (TmpResult is TGpImage) then begin
...
begin
  ...
  for I:= 0 to Count - 1 do begin
    ...
    if Assigned(CellObj) then
      with CellObj do
        if ColSpan > 0 then {skip the dummy cells} begin
          Wd:= 0;
          for J:= I to ColSpan + I - 1 do
            Inc(Wd, Widths[J]); {accumulate column widths}
          if Span = RowSpan then begin
...
begin
  ...
  if (YO + RowSpanHeight >= ARect.Top) and (YO < ARect.Bottom) then
    for I:= 0 to Count - 1 do begin
      CellObj:= TCellObj(Items[I]);
      if Assigned(CellObj) then
        with CellObj do begin
          if (Cell.Count > 0) then begin
            Padding:= CellPadding + CellSpacing;
            if Cell.BkGnd then begin
    ...
    with TAttribute(Attr[I]) do
      case Which of
        ...
        AlignSy:
          ...
          else if CompareText(Name, 'LEFT') = 0 then begin
            Justify:= Left;
            Float:= True;
          end
          else if CompareText(Name, 'RIGHT') = 0 then begin
            Justify:= Right;
            Float:= True;
          end;
        CellSpacingSy:
        ...
        HeightSy:
          if (Pos('%', Name) > 0) and (ACell = Master) then begin
...
begin
  if not BkGnd and (MyCell.BkGnd) then begin {Transfer any Background colors}
    BkGnd:= True;
    BkColor:= MyCell.BkColor;
  end;

  RowCount:= Rows.Count;
  if not ListsProcessed then begin {put dummy cells in rows to make up for ColSpan > 1}
    NumCols:= 0;
    AnyAbsolute:= False;
    for Rw:= 0 to RowCount - 1 do begin
      with TCellList(Rows[Rw]) do begin
        InitializeRow;
        for Cl:= Count - 1 downto 0 do
          with TCellObj(Items[Cl]) do begin
            if WidthAttr > 0 then begin
              if not AsPercent then AnyAbsolute:= True;
            end;
            if Self.BkGnd and not Cell.BkGnd then {transfer bgcolor to cells} begin
              Cell.BkGnd:= True;
          ...
          if Count > Cl then
            if Assigned(Items[Cl]) then
              with TCellObj(Items[Cl]) do begin
                ...
                if RowSpan > 1 then
                  for K:= Rw + 1 to Rw + RowSpan - 1 do begin {insert dummy cells in following rows if RowSpan > 1}
...
begin
  ...
  while More do begin
    ...
    for J:= 0 to Rows.Count - 1 do
      with TCellList(Rows[J]) do begin
        for I:= 0 to Count - 1 do
          if Assigned(Items[I]) then
            with TCellObj(Items[I]) do begin
              More:= More or (ColSpan > Span); {set if need another iteration}
              if ColSpan = Span then begin
                Cell.MinMaxWidth(Canvas, Min, Max);
                Inc(Min, Addon);
                Inc(Max, Addon);
                if Span = 1 then begin
                  if not AsPercent and (WidthAttr > 0) then begin
                  ...
                  TotalMinWidth:= 0;
                  TotalMaxWidth:= 0;
                  for N:= I to I + ColSpan - 1 do begin {find the current totals for the span}
                  ...
                  if (TotalMaxWidth < Max) then
                    if TotalMaxWidth > 0 then begin {increase the sub widths to match the span}
                      D:= Max - TotalMaxWidth;
                      for N:= I to I + ColSpan - 1 do {increase the sub widths to match the span}
...
begin
  ...
  else {make table fit} begin
    D:= TotalMaxWidth - TotalMinWidth;
    W:= TablWidth - TotalMinWidth;
    if D > 0 then {expand only those columns with some slop in them} begin
      for N:= 0 to NumCols - 1 do begin
            ...
            with TCellObj(Items[I]) do begin
              ...
              if ColSpan = Span then begin
                ...
                if Span = 1 then begin
                  ...
                  TotalPC:= 0;
                  NonPC:= 0;
                  for N:= I to I + ColSpan - 1 do begin {Total up the pertinant column widths}
                    Inc(TotalMaxWidth, MaxWidths[N]);
                    Inc(TotalMinWidth, MinWidths[N]);
                  ...
                  if (PCWidth > 0) or (TotalPC > 0) then begin {manipulate for percentages}
                    ...
                    if ExcessMin > 0 then begin
                      if NonPC > 0 then {split excess over non-specified cells} begin
...
{First calculate everything assuming the data entered is perfectly correct}
  ...
  if Delta < 0 then {table is too small} begin
    if Unsp > 0 then begin
      if (UnspDiff > 0) and (UnspDiff >= Abs(Delta) div 2) then
      {increase the unspecified columns widths prop to Max, Min unless the difference is trivial} begin
  ...
  else if Delta > 0 then {calculated table is too large} begin
    Count:= 0;
  {make one or more trial run to see what happens when shrinking the columns
   that can be shrunck.  May hit another MinWidth situation}
    repeat
      NoChange:= True;
      for I:= 0 to NumCols - 1 do
        if (Percents[I] > 0) and not UseMin[I] then begin
          W:= Widths[I] - MulDiv(Delta, Percents[I], PCNotMinWid);
          if W < MinWidths[I] then begin {new width is smaller than MinWidth, make adustments}
            UseMin[I]:= True;
            NoChange:= False;
...
{Figure the width of each column}
  if UseAbsolute and not Specified then begin
  ...
  if (TotalMinWidth >= NewWidth) then begin {table won't fit, use minimun widths}
  ...
  if UsesPercents then begin {find the largest width that will accomodate the %'s}
    Residual:= 0;
    W1:= 0;
    W2:= 0;
    for I:= 0 to NumCols - 1 do
    ...
    Total:= IntMax(W1, W2);
    if Total <= NewWidth then begin {a fit is found using percents and maxwidths}
      if WidthAttr > 0 then
    ...
    repeat {with the above possibilites taken care of, we can assume the final
      ...
      for I:= 0 to NumCols - 1 do begin
        if Percents[I] > 0 then begin
          ...
          if W < MinWidths[I] then {but it must be > MinWidth} begin {eliminate the percentage value as not achievable}
            Percents[I]:= 0;
      ...
      if Total + Residual <= NewWidth then begin {a solution with at least some percentages can be found}
        Done:= True;
        TotalMaxWidth:= 0;
      ...
      else {it doesn't fit screen, reduce percentages and try again} begin
...
{Find Table Width}
  ...
  {Find the height of each row allowing for RowSpans}
      ...
      for J:= 0 to Rows.Count - 1 do
        with TCellList(Rows[J]) do begin
          ...
          if Span = 1 then
            ...
            if H > TotalHt then {apportion the excess over the rows} begin
              Addon:= ((H - TotalHt) div Span);
              AddedOn:= 0;
              for K:= J to J + Span - 1 do begin
                Inc(Heights^[K], Addon);
                Inc(AddedOn, Addon);
              end;
    ...
    if TopCaption then begin {layout the caption}
    ...
    if HeightAttr > 0 then begin
      if HtAsPercent then
        with ThtmlLite(ParentSectionList.TheOwner) do begin
      ...
      if Rows.Count > 0 then begin
        ...
        if TotalHt < NewHeight then begin
          Addon:= (NewHeight - TotalHt) div Rows.Count;
          Sum:= 0;
          for J:= 0 to Rows.Count - 2 do begin
            Inc(Heights^[J], Addon);
            Inc(Sum, Heights^[J]);
          end;
    ...
    for J:= 0 to Rows.Count - 1 do
      with TCellList(Rows[J]) do begin
        ...
        for I:= 0 to Count - 1 do
          if Assigned(Items[I]) then
            with TCellObj(Items[I]) do begin {find the actual height, Ht, of each cell}
              Ht:= 0;
...
begin
  ...
  if (YO + DrawHeight >= ARect.Top) and (YO < ARect.Bottom) then begin
      ...
      with TCellList(Rows[J]) do begin
        for I:= 0 to Count - 1 do begin
          if Assigned(Items[I]) then
            with TCellObj(Items[I]) do begin
              if (X >= XX) and (X < XX + Wd)
                and (Y >= YY) and (Y < YY + Ht) then begin
...
begin
  Result:= -1;
  if (Y <= SectionHeight) then begin
    TableOK:= (X >= DrawX) and (X <= TableWidth + DrawX);
    CaptionOK:= (X >= DrawX + CaptionIndent - Indent) and (X <= DrawX + CaptionWidth + CaptionIndent - Indent);
    if TopCaption then
      if Y < CaptionHeight then begin
        if CaptionOK then begin
          Result:= Caption.Cell.FindCursor(Canvas, X, Y, XR, YR, CaretHt, SCell, InText);
        end;
      end
      else begin
        if TableOK then begin
...
begin
  ...
  else {//Prev , iterate in reverse} begin
...
begin
  ...
  if Assigned(AnURL) and (Length(AnURL.Url) > 0) then begin
...
var
  ...
  S: string[15];
begin
  if L.Find(ClearSy, T) then begin
    S:= LowerCase(T.Name);
...
begin
  if Len > 0 then begin
    Buff[Len]:= #0;
    if Assigned(XP) then {XP = Nil when printing} begin
      Last:= 0; {to prevent warning msg}
      SIndexList:= TFreeList.Create;
      for I:= 0 to Len - 1 do begin
        if (I = 0) or (XP^[I] <> Last + 1) then begin
          IO:= IndexObj.Create;
          IO.Pos:= I;
          IO.Index:= XP^[I];
          SIndexList.Add(IO);
...
{Delete leading spaces or multiple spaces}
  if not NoBreak then begin
    if ((Len = 0) or (Buff[Len - 1] = ' ')) and (S.S[1] = ' ') then begin
      if Length(S.S) = 1 then Exit;
      Remove(1);
    end;
  end
  else begin
    if ((Len = 0) or (Buff[Len - 1] in [#5, #160, ' '])) and (S.S[1] = #5) then begin
      if Length(S.S) = 1 then Exit;
      Remove(1)
    end;
    I:= Pos(' '#5, S.S);
  ...
  if Len > 0 then
    for I:= 0 to Len - 1 do begin
      if (not (Buff[I] in [#4, #7])) or not (Images.FindImage(I).ObjAlign in [ALeft, ARight]) then
        Break;
      if (I = Len - 1) and (Length(S.S) > 0) and (S.S[1] in [' ', #160]) then begin
        if Length(S.S) = 1 then Exit;
        Remove(1)
      end;
    end;
...
begin
  Result:= Justify;
  if Len = 0 then {need to have at least one space} begin
...
begin
  ...
  with ThlParser(ParentSectionList.Parser) do begin
...
begin
  if Sy in [BSy, BEndSy, ISy, IEndSy, EmSy, EmEndSy, StrongSy, StrongEndSy,
    USy, UEndSy, CiteSy, CiteEndSy, VarSy, VarEndSy] then begin
    FO:= TFontObj(Fonts[Fonts.Count - 1]);
...
begin
  ...
  else if Which = SelectSy then begin
...
begin
  ...
  while True do begin
    ...
    if Cnt + I >= Max then {I has been initialized} begin
      Cnt:= Max;
      Break;
    end
    else Inc(Cnt, I);

    if not Picture then begin
      if I < J then Break;
...
begin
  ...
  while True do begin
    ...
    J2:= Images.GetImageCountAt(Start - Buff);
    J3:= TFormControlList(FormControls).GetControlCountAt(Start - Buff);
    if J2 = 0 then begin {next is an image}
      ImgWidth:= Images.GetWidthAt(Start - Buff, Align, HSpc);
      if Align in [ALeft, ARight] then begin
        FlObj:= Images.FindImage(Start - Buff);
        if ImageAtStart then begin
          IMgr.Update(Y, FlObj);
    ...
    if not Picture then {Picture has been initialized} begin
...
begin
  ...
  for I:= 0 to Images.Count - 1 do {call drawlogic for all the images} begin
    Obj:= Images[I];
    if (Obj is TImageObj) then
      with TImageObj(Obj) do begin
...
begin
  ...
  while N > 0 do begin
    ...
    if J = 0 then {it's and image} begin
    ...
    {Here we count floating images as 1 char but do not include their width,
      ...
      if not (Align in [ALeft, ARight]) then begin
        Result:= Result + Wid + 2 * HSpc;
      end;
      Dec(N); {image counts as one char}
      Inc(Start);
    end
    else if J1 = 0 then begin
...
var
  ...
  begin
    Result:= 0;
    if (ClearAttr <> clrNone) then begin {may need to move down past floating image}
  ...
  begin
    ...
    for I:= 0 to NN - 1 do begin
      if not (P^ in [#2, #4, #6, #7]) then begin {check for the no character case}
        NoChar:= False;
        Break;
      end;
      Inc(P);
    ...
    for I:= 0 to NxImages.Count - 1 do begin
...
begin
  ...
  while not Finished do begin
    ...
    if (Lines.Count = 0) then begin {may need to move down past floating image}
      Tmp:= GetClearSpace;
      if Tmp > 0 then begin
        LR.LineHt:= Tmp;
    ...
    if N = Max then begin {Do the remainder}
      LineComplete(N);
      Finished:= True;
    end
    else begin
      P:= PStart + N - 1;
      if (P^ = ' ') then begin {move past spaces so as not to print any on next line}
        while (N < Max) and ((P + 1)^ = ' ') do begin
          Inc(P);
          Inc(N);
        end;
        LineComplete(N);
      ...
      else if (N < Max) and ((P + 1)^ in [#2, #4]) then {an image or control} begin
        LineComplete(N);
        Finished:= False;
      end
      else begin {non space, wrap it by backing off to previous space or image}
        while not (P^ in [' ', #2, #4]) and (P > PStart) do Dec(P);
        if P = PStart then begin {no space found, forget the wrap, write the whole word and any
               spaces found after it}
          P:= PStart + N - 1;
          while (P <> Last) and not ((P + 1)^ in [' ', #2, #4]) do begin
            Inc(P);
          end;
          while (P <> Last) and ((P + 1)^ = ' ') do begin
            Inc(P);
          end;
  ...
  Curs:= StartCurs + Len;
  if Level > 0 then
  {for lists, clear left floating images} begin
...
var
  ...
  var
    ...
    begin
      ...
      if (MySelB <= C) and (MySelE > C) then begin
        Result:= True;
        LongCount:= MySelE - C;
      end
  ...
  begin {Y is at bottom of line here}
    ...
    while Cnt > 0 do begin
      ...
      OHang:= FO.OverHang;
      if J2 = -1 then begin {it's an image}
        Obj:= Images.FindImage(Start - Buff);
        FO:= Fonts.GetFontObjAt(Start - Buff, Index);
        if Obj is TImageObj then begin
          if Obj.ObjAlign in [ALeft, ARight] then begin
            if ImageAtStart then begin
        ...
        if Assigned(Ctrl.FControl) then
          with Ctrl, FControl do begin
            ...
            if Ctrl is TRadioButtonFormControlObj then
              with TRadioButtonFormControlObj(Ctrl) do begin
        ...
        Move(Start^, S[1], I);
        J:= Pos(#160, S);
        while J > 0 do {substitute spaces for #160} begin
          S[J]:= ' ';
          J:= Pos(#160, S);
        end;
        if Self is TPreformated then begin {so will clip in Table cells}
      ...
      {Put in a dummy caret to show character position}
        if ParentSectionList.ShowDummyCaret and not Inverted
          and (MySelB = Start - Buff) then begin
...
begin
  ...
  if (Len > 0) and (Y - YOffset + DrawHeight >= ARect.Top) and (Y - YOffset < ARect.Bottom) then begin
    DC:= Canvas.Handle;
...
begin
  ...
  MySelE:= ParentSectionList.SelE - StartCurs;
  for I:= 0 to Lines.Count - 1 do
    with LineRec(Lines.Items[I]) do begin
      Strt:= Start - Buff;
...
{First, check to see if in an image}
  if (Images.Count > 0) and
    Images.PtInImage(X, YValue + Y, IX, IY, Posn, IMap, UMap, MapItem, ImageObj) then begin
    ParentSectionList.ActiveImage:= ImageObj;
    if Assigned(ImageObj.MyFormControl) then begin
      FormControl:= ImageObj.MyFormControl;
      Result:= True;
      FormControl.XTmp:= IX;
      FormControl.YTmp:= IY;
    end
    else if UMap then begin
      if MapItem.GetURL(IX, IY, UrlTarg) then
        Result:= True;
    end
    else begin
      FO:= Fonts.GetFontObjAt(Posn, Index);
      if FO.UrlTarget.Url <> '' then begin {found an URL}
        Result:= True;
...
begin
  ...
  with LR do begin
    ...
    XR:= DrawX + W;
    if L < Ln then begin {check to see if passed 1/2 character mark}
      Delta:= FindTextWidth(Canvas, Start + L, 1, False);
      if Width > W + (Delta div 2) then begin
        Inc(L);
        Inc(XR, Delta);
      end;
    end
...
begin
  ...
  if MatchCase then begin {case sensitive search}
    P:= StrPos(Buff + I, PC);
    if Assigned(P) then
      Result:= StartCurs + (P - Buff);
  end
  else begin {Caseless search}
    UCh:= PC^;
    ...
    S1:= StrPas(PC);
    S2[0]:= chr(LenPC);
    while Assigned(P) and (StrLen(P) >= LenPC) do begin
...
begin
  ...
  for I:= SIndexList.Count - 1 downto 0 do begin
    IO:= PosIndex[I];
    if IO.Pos <= DocPos - StartCurs then begin
...
begin
  ...
  if not Prev then begin
    ...
    IOPrev:= PosIndex[0];
    if SourcePos <= IOPrev.Index then begin //in this section but before the start of Document text
      Result:= StartCurs;
      Exit;
    end;

    for I:= 1 to SIndexList.Count - 1 do begin
      IO:= PosIndex[I];
      if (SourcePos >= IOPrev.Index) and (SourcePos < IO.Index) then begin //between IOprev and IO
  ...
  else {//prev  -- we're iterating from the end of TSectionList} begin
    IOPrev:= PosIndex[0];
    if SourcePos < IOPrev.Index then Exit; //before this section

    I:= SIndexList.Count - 1;
    IO:= PosIndex[I];
    if SourcePos > IO.Index + (Len - 1) - IO.Pos then begin //SourcePos is after the end of this section
