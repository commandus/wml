...
begin {Getch}
  repeat {in case a comment immediately follows another comment}
   {comments may be either '<! stuff >' or '<!-- stuff -->'  }
    Comment:= False;
    GetchBasic;
    if (Ch = '<') and not InScript then begin
      if (Peek = '!') then begin
        GetChBasic;
        Comment:= True;
        GetChBasic;
...
begin
  ...
  while not (Ch in [' ', Tab, ^M, '>', '%', EofChar]) do begin
    S:= S + Ch;
    GetCh;
  end;
  SkipWhiteSpace;
...
begin
  ...
  try
    NextCh;
    if Ch = '#' then begin {look for char #}
      NextCh;
      while I <= 5 do begin
        if not (Ch in ['0'..'9']) then begin
          ...
          if S <> '' then begin
            ...
            else if (byte(N) in [9, 10, 32..255]) then begin
    ...
    else while I <= 6 do begin
        if not (Ch in ['A'..'Z', '0'..'9']) then {note: Ch is always upper case} begin
          if Ch = ';' then NextCh;
          for J:= 1 to MaxChars do
            if S = OddCharStr[J] then begin
              if J <= Lim then
                if S = 'hellip' then begin
                  LCToken.S:= '...';
...
begin
  ...
  SaveSy:= Sy;
  GetCh;
  while not (Ch in [Term, EofChar]) do begin
    if LCh <> ^M then begin
      if IsOddChar then begin
        S:= S + LCToken.S;
        LCToken.Clear;
      end
      else begin
...
begin
  ...
  SkipWhiteSpace;
  St:= '';
  if GetID(St) then begin
    for I:= 1 to MaxAttr do
      if St = Attrib[I] then begin
        Sym:= AttribSym[I];
        Break;
      end;
  end
...
begin
  ...
  else if not (Ch in ['A'..'Z']) then begin {an odd '<'}
    Sy:= TextSy;
  ...
  while not Done do
    case Ch of
      'A'..'Z': begin
          if Length(Compare) < 255 then begin
            Inc(Compare[0]);
  ...
  for I:= 1 to MaxRes do
    if Compare = ResWords[I] then begin
      if not EndTag then
        Sy:= ResSy[I]
      else
        if I <= MaxEndRes then
  ...
  SkipWhiteSpace;
  Value:= 0;
  if ((Sy = HeadingSy) or (Sy = HeadingEndSy)) and (Ch in ['1'..'6']) then begin
    Value:= ord(Ch) - ord('0');
    GetCh;
  end;
  ...
  if (Buffer = BuffEnd) and (Sy in [PEndSy, BRSy]) then begin
    LCh:= ' ';
    Ch:= ' ';
  end
  else
...
begin
  ...
  while (Length(LCToken.S) < 100) and
    (LCh in [^M, ' '..'%', ''''..';', '=', '?'..#255, '>']) do
    if not PreFormat and ((LCh = ' ') or (LCh = ^M)) then begin
      if NoBreak then
  ...
  if Length(LCToken.S) > 0 then begin
    Sy:= TextSy;
    IsText:= True;
  end
  else IsText:= False;
...
begin
  if StackIndex < MaxStack then begin
    Inc(StackIndex);
...
var
  ...
  procedure Next1;
    ...
    begin
      ...
      GetCh;
      Sy:= CommandSy;
      while not (LCh in [' ', ^M, Tab, '>']) do begin
        Token:= Token + LCh;
        GetCh;
      end;
    ...
    begin
      while (Length(Token) < 100) and not (LCh in [^M, '<', '&', EofChar]) do begin
        Token:= Token + LCh;
        GetCh;
      end;
  ...
  begin {already have fresh character loaded here}
    ...
    LCToken.Clear;
    if LCh = EofChar then Sy:= EofSy
    else if LCh = ^M then begin
      Sy:= EolSy;
      GetCh;
    end
    else if LCh = '<' then begin
      GetTag1;
      Exit;
    end
    else if IsOddChar then
...
begin
  Next1;
  S:= '';
  while (Sy <> TextAreaEndSy) and (Sy <> EofSy) do begin
    case Sy of
      TextSy: S:= S + Token;
      EolSy: begin
          TxtArea.AddStr(S);
...
begin
  ...
  if Attributes.Find(AlignSy, T) then begin
...
begin
  if InHref then {see if we're in an href} begin
...
var
  ...
  begin
    Result:= Default;
    if Attributes.Find(VAlignSy, T) then begin
      S:= LowerCase(T.Name);
  ...
  begin
    if Assigned(SectionList) then begin
  ...
  while (Sy <> TableEndSy) and (Sy <> EofSy) do
    case Sy of
      TDSy, THSy: begin
          ...
          if not Assigned(Row) then {in case <tr> is missing} begin
            Row:= TCellList.Create;
            RowJustify:= NoJustify;
            RowVAlign:= AMiddle;
          end
          else AddSection;
          if Sy = THSy then begin
...
begin
  ...
  Selected:= False;
  InOption:= False;
  while not (Sy in [SelectEndSy, EofSy]) do begin
    case Sy of
...
begin
  ...
  try
    ...
    Next;
    while (Sy <> MapEndSy) and (Sy <> EofSy) and (ErrorCnt < 3) do begin
...
var
  ...
  begin
    if Pos + Length(S) >= Size then begin {Delphi 2,3, add to buffer}
  ...
  procedure Next1;
    ...
    begin
      ...
      Count:= 0;
      while (Ch in ['A'..'Z', '/']) and (Count <= 6) do begin
        LCToken.AddChar(LCh, SIndex);
        GetCh;
        Inc(Count);
      end;
  ...
  begin {already have fresh character loaded here}
    ...
    if LCh = EofChar then Sy:= EofSy
      ...
      Sy:= TextSy;
      while (Length(LCToken.S) < 100) and not (LCh in [^M, '<', EofChar]) do begin
        LCToken.AddChar(LCh, SIndex);
        GetCh;
      end;
    end;
...
begin
  try
    if Assigned(AScript) then begin
      ...
      try
        Next1;
        while (Sy <> ScriptEndSy) and (Sy <> EofSy) do begin
...
var
  ...
  begin
    ...
    for I:= 0 to Attributes.Count - 1 do
      with TAttribute(Attributes[I]) do
        case Which of
          ...
          FaceSy:
            if (Sy <> BaseFontSy) and (Name <> '') then begin
              FaceName:= Name;
              K:= Pos(',', FaceName);
              if K > 0 then
    ...
    if ((Sy <> BasefontSy) or (SectionList.Count = 0) and
      ...
      with FontStack[StackIndex] do begin
        if Colr in FontResults then
          Color:= NewColor or $2000000;
        if Siz in FontResults then begin
          if Pre then NewSize:= PreFontConv[NewSize]
          else NewSize:= FontConv[NewSize];
          ...
          '<': begin
              ...
              case Sy of
                ...
                BSy, ISy, BEndSy, IEndSy, EmSy, EmEndSy, StrongSy, StrongEndSy,
                  ...
                  SSy, SEndSy, StrikeSy, StrikeEndSy: begin
                    Section.AddTokenObj(S, NoBreak);
                    S.Clear;
                    case Sy of
                ...
                FontEndSy:
                  if StackIndex > InitialStackIndex then begin
                    PopFont;
                    Section.AddTokenObj(S, NoBreak);
                    S.Clear;
                    TSection(Section).ChangeFont(MasterList, FontStack[StackIndex]);
                    ...
                    with TAttribute(Attributes[I]) do
                      case Which of
                        ...
                        HRefSy: begin
                            ...
                            S.Clear;
                            if InHref then DoAEnd;
                            if Name <> '' then {also have a string} begin
...
begin
  case Sy of
    ...
    PSy, PEndSy: begin
        ...
        NoPSpace:= True;
        Next;
        while Sy in [PSy, PEndSy] do begin {recognize only the first <p>}
    ...
    BSy, ISy, BEndSy, IEndSy, EmSy, EmEndSy, StrongSy, StrongEndSy,
      USy, UEndSy, CiteSy, CiteEndSy, VarSy, VarEndSy,
      SubSy, SubEndSy, SupSy, SupEndSy, SSy, SEndSy, StrikeSy, StrikeEndSy: begin
        case Sy of
          BSy, StrongSy: CurrentStyle:= CurrentStyle + [fsBold];
          BEndSy, StrongEndSy: CurrentStyle:= CurrentStyle - [fsBold];
          ISy, EmSy, CiteSy, VarSy: CurrentStyle:= CurrentStyle + [fsItalic];
    ...
    TTSy, CodeSy, KbdSy, SampSy: begin
        if PushNewFont then begin
          with FontStack[StackIndex] do begin
            Name:= MasterList.PreFontName;
            SetNormalSize(MasterList, 10);
    ...
    TTEndSy, CodeEndSy, KbdEndSy, SampEndSy, FontEndSy, BigEndSy, SmallEndSy: begin
        PopFont;
        if Assigned(Section) then
          TSection(Section).ChangeFont(MasterList, FontStack[StackIndex]);
        Next;
...
begin
  Result:= DefCharset;
  for I:= 1 to MaxX do
    if Pos(XTable[I].S, Lowercase(Content)) > 0 then begin
...
{$IFDEF ver100_plus}
  if (Sender is ThtmlLite) and (CompareText(HttpEq, 'content-type') = 0) then begin
  ...
  repeat
    case Sy of
      TextSy, BRSy, PSy, PEndSy,
        ...
        DoCommonSy(Level);
      BodySy: begin
          if SectionList.Count = 0 then {make sure we're at beginning} begin
            Section.Free; {Will start with a new section}
            for I:= 0 to Attributes.Count - 1 do
              with TAttribute(Attributes[I]) do
                case Which of
                  BackgroundSy: MasterList.SetBackgroundBitmap(Name);
                  TextSy: if GetColor(Name, Val) then begin
                  ...
                  OLinkSy: if GetColor(Name, Val) then begin
      ...
      LiSy:
        if Level = 0 then begin
          ...
          while Sy in [TextSy, NoBrSy, NoBrEndSy, WbrSy, BSy, ISy, BEndSy, IEndSy,
            ...
            SmallEndSy, BigSy, SmallSy, ASy, AEndSy] do begin
            DoCommonSy(1);
          end;
          SectionList.Add(Section);
          Section:= nil;
