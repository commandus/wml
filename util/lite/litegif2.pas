...
begin
  ...
  try
    ...
    try
      ...
      for I:= 0 to Result.FNumFrames - 1 do begin
...
begin
  ...
  if (FVisible) and (FNumFrames > 0) then begin
    with Frames[FCurrentFrame] do begin
...
begin
  ...
  if (FCurrentFrame = FNumFrames) then begin
    if (FNumIterations > 0) and (CurrentIteration >= FNumIterations) then begin
      SetAnimate(False);
      Exit;
    end;
...
begin
  ...
  FAnimate:= AAnimate;
  if AAnimate and (FNumFrames > 1) then begin
    CurrentInterval:= IntMax(Frames[FCurrentFrame].frDelay, 1);
