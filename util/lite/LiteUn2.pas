...
var
  ...
  begin
    I:= Pos('%', FName);
    while (I > 1) and (I <= Length(FName) - 2) do begin
...
begin
  with Canvas do begin
    ...
    S1:= '';
    while (Length(S) > 0) and (Y + Step <= Y2) do begin
      ...
      if TextWidth(S1 + S2) <= Width then begin
        S1:= S1 + S2 + ' ';
      end
      else begin
        if S1 <> '' then begin
...
begin
  ...
  Y2:= IntMin(Y2, BotLim);
  with Canvas do begin
    with SectionList as TSectionList do begin
...
begin
  I:= IndexOf(S);
  if I >= 0 then
    with Objects[I] as TBitmapItem do begin
      Dec(UsageCount);
...
begin
  for I:= Count - 1 downto 0 do
    with (Objects[I] as TBitmapItem) do begin
      if (AccessCount > N) and (UsageCount <= 0) then begin
        Delete(I);
        Free;
      end;
    end;
...
begin
  for I:= Count - 1 downto 0 do begin
    ...
    with Tmp do begin
      Inc(AccessCount);
      if (AccessCount > MaxCache) and (UsageCount <= 0) then begin
        Delete(I);
...
begin
  for I:= 0 to Count - 1 do
    if TAttribute(Items[I]).which = Sy then begin
      Result:= True;
      T:= Items[I];
      Exit;
    end;
...
begin
  ...
  with Areas do
    for I:= 0 to Count - 1 do
      if PtInRegion(THandle(Objects[I]), X, Y) then begin
        if Strings[I] <> '' then {could be NoHRef} begin
    ...
    with TAttribute(Attrib[I]) do
      case Which of
        ...
        CoordsSy: begin
            ...
            Cnt:= 0;
            GetSubStr;
            while (S1 <> '') and (Cnt <= MAXCNT) do begin
...
begin
  ...
  IT:= KindOfImageFile(Filename);
  if (IT = png) and GDIPlusActive then begin
    W:= Filename;
...
begin
  ...
  if (IT = png) and GDIPlusActive then begin
    try
      ...
      CloseFile(F);
      Result:= TgpImage.CreateWFile(Filename);
...
begin
  ...
  { monochrome DC}
    ...
    if (BM.bmBitsPixel > 1) or (BM.bmPlanes > 1) then begin
    { create some DCs to hold temporary data}
...
begin
  ...
  else if (Justify = Right) then begin
    with IR do begin
...
begin
  ...
  for I:= 0 to L.Count - 1 do
    with IndentRec(L.Items[I]) do begin
...
begin
  if Level > CurrentLevel then begin
    for I:= CurrentLevel + 1 to Level do {in case we skip a level} begin
      IR:= IndentRec.Create;
      with IR do begin
        Lev:= I;
        YT:= Y;
  ...
  else if (Level < CurrentLevel) and (CurrentLevel > 0) then begin
    for I:= 0 to L.Count - 1 do
      with IndentRec(L.Items[I]) do
...
var
  ...
  IR: IndentRec;
begin
  if Level = CurrentLevel + 1 then begin
    begin
      IR:= IndentRec.Create;
      with IR do begin
        Lev:= Level;
  ...
  else if (Level = CurrentLevel - 1) and (CurrentLevel > 0) then begin
    for I:= 0 to L.Count - 1 do
      with IndentRec(L.Items[I]) do
        if Lev > Level then begin
          Lev:= 0;
...
begin
  S:= S + C;
  if Length(S) > MaxIndex then begin
...
begin
  K:= Length(S);
  S:= S + T.S;
  if K + Length(T.S) > MaxIndex then begin
